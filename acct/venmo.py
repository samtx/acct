...
def parse_venmo_currency_str(currency_str) -> float:
    ...
    """
    parsed_str = (
        currency_str.replace("$", "").replace(",", "").replace(" ", "").replace('"', "")
    )
...
class Venmo:
        ...
        with open(csv_file, "r", encoding="utf-8") as f:
            ...
            for row in reader_list:
                ...
                data = {
                    ...
                    "to": row["To"],
                    "note": row["Note"],
                    "amount": parse_venmo_currency_str(row["Amount (total)"])
                    if row["Amount (total)"]
                    else 0.0,
                    "fee": parse_venmo_currency_str(row["Amount (fee)"])
                    if row["Amount (fee)"]
                    else 0.0,
    ...
    def to_lunchmoney(
        ...
        if transaction.type == TransactionType.payment:
            payee = (
                transaction.from_ if transaction.to == venmo_user else transaction.to
            )
